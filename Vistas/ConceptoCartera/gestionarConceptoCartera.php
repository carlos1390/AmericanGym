				...
				<div class="row-fluid">
					...
					<div class="span12 well">
						<div class="page-header">
							<h4>Gestionar concepto cartera</h4>
						</div>

	<?php
    extract($_GET);
     if (empty($idconceptocartera)){
     ?>
     <form action='../../Negocios/ConceptoCartera/insertar.php' method="POST" >
     <label>Nombre:</label> <input type="text" name="nombre">
     <br>
     <label>Totalconcepto:</label> <input type="text" name="totalconcepto">
     <br>
    ...
    else
  {
    ...
    echo  "<br>";
  	...
  	$conceptocarteratemp= new ConceptoCarteraDAO(); 
  	//echo $idpersona;
  	$conceptocarteratemp->obteberconceptocartera($idconceptocartera);
  	for ($i = 0; $i < count($conceptocarteratemp->idconceptocartera); $i++)
     {
        $nombretemp=$conceptocarteratemp->nombre[$i];
        $totalconceptotemp=$conceptocarteratemp->totalconcepto[$i];
      }

  	?>
  	<form action="../../Negocios/ConceptoCartera/actualizar.php" method="post">
  		<input type="hidden" name="id" value="<?php echo $idconceptocartera;?> ">
      <label>Nombre:</label><input type="text" name="nombre" value="<?php echo $nombretemp;?>">
  	<br>
  	<label>totalconcepto:</label><input type="text" name="totalconcepto" value="<?php echo $totalconceptotemp;?>">
  	<br>

        ...
        <div class="row-fluid">
          ...
          <div class="span12 well">
            <div class="page-header">
              <h4>Gestion Unidad De Medida</h4>
            </div>

<?php
	extract($_GET);
	if(empty($idunidaddemedida)) {
		?>
		<form action="../../Negocios/UnidadMedida/Insertar.php" method="POST">
		...
		$UnidaddeMedidatemp= new UnidaddeMedidaDao();
		$UnidaddeMedidatemp->ObtenerUnidaddeMedida($idunidaddemedida);
		for ($i=0; $i < count($UnidaddeMedidatemp->idunidademedida);$i++) { 

			$nomretemp=$UnidaddeMedidatemp->nombre[$i];
		}
		?>
		<form action="../../Negocios/UnidadMedida/Actualizar.php" method="post">
		<input type="hidden" name="id" Value="<?php echo $idunidaddemedida;?>"> 
  	<label>Nombre:</label><input type="text" name="nombre" value="<?php echo $nomretemp;?>">
  	<br>

 ...
 <div class="row-fluid">					
	...
	<div class="span12 well">
		<div class="page-header">
			<h4>Gestionar Personas</h4>
		</div>
	    <form action="" class="form-inline">	
			<div class="input-append">
		    	 <a href='GestionarPersona.php' class="btn">Crear Nuevo</a>
			</div>				
	    </form>
    </div>
    <!--Tabla-->
		...
		<table class="tablesorter table table-bordered table-striped table-hover table-condensed ">
	        ...
	        for ($i=0; $i <count($Persona->NumeroDocumento) ; $i++) { 
      	    ?>
      	    <tr id="fila-<?php echo $Persona->NumeroDocumento[$i]; ?>">
               <td> <?php echo $Persona->NumeroDocumento[$i]; ?></td>
                <td> <?php echo $Persona->Nombres[$i]; ?></td>
                <td> <?php echo $Persona->Apellidos[$i]; ?></td>
                <td> <?php echo $Persona->NombreGenero[$i]; ?></td>
                <td> <?php echo $Persona->FechaNacimiento[$i]; ?></td>
                <td><?php echo  $Persona->NombreMunicipio[$i];?></td>    
				<td><?php echo  $Persona->Direccion[$i];?></td>    
				<td><?php echo  $Persona->Telpersonal[$i];?></td>    
				<td><?php echo  $Persona->FechaIngreso[$i];?></td>    
				<td><?php echo  $Persona->NombreHorario[$i];?></td>
                <td><a href='GestionarPersona.php?idpersona=<?php echo $Persona->NumeroDocumento[$i];?>'>Editar</a></td>
                <td>
                    <a onClick="EliminarPersona(<?php echo $Persona->NumeroDocumento[$i] ?>); return false" 
						href="../../Negocios/Persona/Eliminar.php?idpersona=<?php echo $Persona->NumeroDocumento[$i];?>">
						Eliminar
				    </a>
                </td>
            </tr>
...
function EliminarPersona(persona_id){
	var msg = confirm("Desea eliminar esta Persona?");
	if ( msg ) {
		$.ajax({
			url: '../../Negocios/Persona/Eliminar.php',
			type: "GET",

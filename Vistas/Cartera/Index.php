	...
	<div class="span12 well">
	    ...
	    <form action="" class="form-inline">	
			<div class="input-append">
		    	 <a href='GestionarCartera.php' class="btn">Crear Nuevo</a>
			</div>				
	    </form>
    </div>
    <!--Tabla-->
		...
		<table class="tablesorter table table-bordered table-striped table-hover table-condensed ">
			...
			$Cartera->ObtenerCarteras();
	        for ($i=0; $i <count($Cartera->id_cartera) ; $i++) { 
      	    ?>
      	    <tr>
               <td> <?php echo $Cartera->fecha_cartera[$i]; ?></td>
                <td> <?php echo $Cartera->numero_documento[$i]; ?></td>
                <td> <?php echo $Cartera->concepto_nombre[$i]; ?></td>
                <td> <?php echo "$ ".number_format($Cartera->abono[$i]); ?></td>
                <td> <?php echo "$ ".number_format($Cartera->saldo[$i]); ?></td>
                <td>
                  <a onclick="return confirm('Realmente desea eliminar este registro?');" 
                	href="../../Negocios/Cartera/Eliminar.php?id_cartera=<?php echo $Cartera->id_cartera[$i];?>">
                	Eliminar
                  </a>
                </td>
            </tr>

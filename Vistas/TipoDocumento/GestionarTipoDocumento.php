	...
	if(empty($idTipoDocumento)) {
		?>
		<form action="../../Negocios/TipoDocumento/Insertar.php" method="POST">
		...
		$TipoDocumentotemp= new TipoDocumentoDao();
		$TipoDocumentotemp->Obtenertipodocumento($idTipoDocumento);
		for ($i=0; $i < count($TipoDocumentotemp->idtipodocumento);$i++) { 

			$nomretemp=$TipoDocumentotemp->nombre[$i];
		}
		?>
		<form action="../../Negocios/TipoDocumento/Actualizar.php" method="post">
		<input type="hidden" name="id" Value="<?php echo $idTipoDocumento;?>"> 
  	<label><h5>Nombre:</h5></label>
  	<input type="text" name="nombre" value="<?php echo $nomretemp;?>">
  	<br>

    ...
    extract($_GET);
     if (empty($ideps)){
     ?>
     <form action='../../Negocios/Eps/insertar.php' method="POST" >
    ...
    else
  {
    ...
    echo  "<br>";
  	...
  	$epstemp= new EpsDAO(); 
  	//echo $idpersona;
  	$epstemp->obtebereps($ideps);
  	for ($i = 0; $i < count($epstemp->ideps); $i++)
     {
        $nombretemp=$epstemp->nombre[$i];
        
     }

  	?>
  	<form action="../../Negocios/Eps/actualizar.php" method="post">
  		<input type="hidden" name="id" value="<?php echo $ideps;?> ">

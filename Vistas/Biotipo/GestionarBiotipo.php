	...
	if(empty($idbiotipo)) {
		?>
		<form action="../../Negocios/Biotipo/Insertar.php" method="POST">
		...
		$Biotipotemp= new BiotipoDao();
		$Biotipotemp->ObtenerBiotipo($idbiotipo);
		for ($i=0; $i < count($Biotipotemp->idbiotipo);$i++) { 

			$nombretemp=$Biotipotemp->nombre[$i];
		}
		?>
		<form action="../../Negocios/Biotipo/Actualizar.php" method="post">
		<input type="hidden" name="id" Value="<?php echo $idbiotipo;?>"> 
